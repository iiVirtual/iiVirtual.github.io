...
function render() {
  ...
  if (route.name === "train") {
    ...
    if (!state.programDays.length) {
      main = `<div class="empty"><strong>No program</strong>Check the JSON inside hypertrophy-standalone.html.</div>`;
    } else {
...
async function loadProgram() {
  let json = getEmbeddedProgram();
  if (json?.days?.length) return json;
  try {
